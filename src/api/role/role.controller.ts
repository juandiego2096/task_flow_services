...
export class RoleController {
  ...
  async createRole(@Body() newRole: createRoleDto): Promise<RoleEntity | HttpException> {
    const roleByName = await this.roleService.getRoleByName(newRole.name);

    if (roleByName) {
      throw new HttpException(`Already exists a role created with name ${newRole.name}`, HttpStatus.FOUND);
    }
  ...
  async getRoleById(@Param('roleId') roleId: number): Promise<RoleEntity | HttpException> {
  ...
  async updateRole(@Request() req, @Param('roleId') roleId: number, @Body() updateRole: createRoleDto) {
