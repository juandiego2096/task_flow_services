...
export class UserCategoryController {
  ...
  async createUserCategory(
    @Body() newUserCategory: createUserCategoryDto,
  ): Promise<UserCategoryEntity | HttpException> {
    const userCategoryByName =
      await this.userCategoryService.getUserCategoryByName(
        newUserCategory.name,
      );

    if (userCategoryByName) {
      throw new HttpException(
        `Already exists a user category created with name ${newUserCategory.name}`,
        HttpStatus.FOUND,
      );
    }
  ...
  async getUserCategoryById(
    ...
    const userCategoryFound =
      await this.userCategoryService.getUserCategoryById(userCategoryId);
  ...
  async updateUserCategory(
    @Request() req,
    @Param('userCategoryId') userCategoryId: number,
    @Body() updateUserCategory: createUserCategoryDto,
  ) {
    ...
    if (!userCategory) {
      throw new HttpException(
        `User category with id  ${userCategoryId} not found`,
