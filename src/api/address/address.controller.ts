...
export class AddressController {
  ...
  async createAddress(@Body() newAddress: createAddressDto): Promise<AddressEntity | HttpException> {
  ...
  async getAddressById(@Param('addressId') addressId: number): Promise<AddressEntity | HttpException> {
  ...
  async updateAddress(@Request() req, @Param('addressId') addressId: number, @Body() updateAddress: createAddressDto) {
    ...
    if (!address) {
      throw new HttpException(`Address with id  ${addressId} not found`, HttpStatus.NOT_FOUND);
    }
