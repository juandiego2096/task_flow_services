...
export class AgentController {
  ...
  async createAgent(@Body() newAgent: createAgentDto): Promise<AgentEntity | HttpException> {
  ...
  async getAgentById(@Param('agentId') agentId: number): Promise<AgentEntity | HttpException> {
  ...
  async updateAgent(@Request() req, @Param('agentId') agentId: number, @Body() updateAgent: createAgentDto) {
    ...
    if (!agent) {
      throw new HttpException(`Agent with id  ${agentId} not found`, HttpStatus.NOT_FOUND);
    }
