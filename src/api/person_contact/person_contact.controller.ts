...
export class PersonContactController {
  ...
  async createPersonContact(@Body() newPersonContact: createPersonContactDto): Promise<PersonContactEntity | HttpException> {
    return await this.personContactService.createPersonContact(newPersonContact);
  }
  ...
  async getPersonContactById(@Param('personContactId') personContactId: number): Promise<PersonContactEntity | HttpException> {
    const personContactFound = await this.personContactService.getPersonContactById(personContactId);
  ...
  async updatePersonContact(@Request() req, @Param('personContactId') personContactId: number, @Body() updatePersonContact: createPersonContactDto) {
    ...
    const personContact = await this.personContactService.getPersonContactById(personContactId);
    if (!personContact) {
      throw new HttpException(`Person contact with id  ${personContactId} not found`, HttpStatus.NOT_FOUND);
    }
